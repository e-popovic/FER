...
public class Vector {
  ...
  public Vector getReflectedVector(Vector normal) {
	//formula iz uputa (3.5) R=(2*UxN)*N-U
	//N=normal, U=this
    Vector refl=new Vector(0,0,0);
    double skalar=(2*this.dotProduct(normal));	//(2*UxN)
    refl=normal.multiple(skalar).sub(this);		//(2*UxN)*N-U
    return refl;
  }
  ...
  public Vector getRefractedVector(Vector normal, double nI) {
    //formula iz uputa (3.6) R= -a * N - b * U
	//N=normal, b=nI, U=this
	//a = (-2*b*cos(alfa) + sqrt(D) )/ 2  
	//D = 4*(b**2 * (cos(alfa))**2 - b**2 +1)    alfa=kut izmedu normale i this
	 this.normalize();
	 normal.normalize();
	 
	 double alfa= this.getAngle(normal);
	 double D=4*(Math.pow(nI,2) * Math.pow(Math.cos(alfa),2) - Math.pow(nI,2) + 1);
	 double a= (-2 * nI * Math.cos(alfa) + Math.sqrt(D))/2;
	 
	 Vector pt1 = normal.multiple(-a);
	 Vector pt2 = this.multiple(-nI);
	 Vector refr = pt1.add(pt2);
